...
class Train:
    def __init__(self, config_file='configs/DAM.yaml', save_dir=None):
        ...
        elif self.dataset_name == 'BDDADataset_seg':
            dataset_class = BDDADataset_seg
    ...
    def train_epoch(self, epoch):
        ...
        with tqdm(self.train_loader, unit='batch', desc=f'Train epoch {epoch}/{self.train_params["no_epochs"]}') as tepoch:
            for idx, sample in enumerate(tepoch):
                ...
                seg_img = seg_img.to(self.device)
                seg_img = seg_img.permute((0,2,1,3,4))
                ...
                loss.backward()
                # for name, param in self.model.seg_encoder.named_parameters():
    ...
    def val_epoch(self, epoch):
        ...
        print(f'Val epoch {epoch:2d} avg_loss: {total_loss.avg:.5f} cc_loss: {total_cc_loss.avg:.5f} sim_loss: {total_sim_loss.avg:.5f}')
        sys.stdout.flush()
