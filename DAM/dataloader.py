...
class BDDADataset_seg(DReyeVEDataset):
    def __init__(self,
                 obs_length,
                 img_size=(224, 224),
                 map_size=(128, 128), 
    ...
    def get_images(self, vid_id, start_idx):
        ...
        for i in range(self.obs_length):
            load_time = time.time()
            img = self.load_image(img_dir_path, start_idx+i)
            obs_img.append(self.base_transform(img))

        obs_tensor = torch.stack(obs_img, dim=0)
        obs_tensor = self.color_jitter(obs_tensor)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_images:
            # obs_img: [16, 3, 224, 224] (T, C, H, W)
            obs_img = self.get_images(vid_id, start_idx).float()

            # seg_img: [16, 3, 224, 224] (T, C, H, W)
        ...
        # print(f'rgb shape:{obs_img.shape}, seg shape:{seg_img.shape}, gt shape:{gt.shape}')
...
class MMAUDataset(Dataset):
    def __init__(self,
        ...
        super(MMAUDataset, self).__init__()

        self.use_images = use_images
        self.dataset_path = '/home/wayne/Documents/MMAU'
        self.obs_length = obs_length
        self.img_size = img_size
        self.sample_list = []
        self.video_info = {}
    ...
    def setup(self):
        ...
        for subset in subsets:
            ...
            for cat in os.listdir(subset_path):
                ...
                for vid in os.listdir(cat_path):
                    ...
                    img_dir = os.path.join(vid_path, 'images')
                    seg_dir = os.path.join(vid_path, 'segmentation')
        ...
        print(f'-> Total videos found: {len(self.video_info)}')
        print(f'-> Total samples generated: {len(self.sample_list)}')

    def get_images_and_segs(self, vid_id, start_idx):
        vid_data = self.video_info[vid_id]
        obs_img = []
        seg_img = []
        original_size = None
        ...
        for i in range(self.obs_length):
            ...
            try:
                ...
                print('ERROR: could not load', seg_path)
                raise(OSError)
